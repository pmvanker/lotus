...
void setup() {
  ...
  mySerial.println("UC start");
}

int x=0, y=0, i=0;   // start top left
...
void loop()
{
  ...
  {
    str_cloude = mySerial.readString();
    mySerial.println(str_cloude);
    mySerial.println(str_cloude.length());
    str_cloude.toCharArray(text, str_cloude.length());

    len = strlen(text) - 1 ;
    if (text[len] != ';')
    {
        mySerial.println("Data Frame Wrong");
    }

    key =  strtok(text,KEY_TER);
    ...
    {
      strcpy(text,"INDORE SMART MSG BOX DESIGNED BY LOTUS");
    }
    mySerial.println(strlen(text));
  }

//  Serial.println(text);
  // Draw the text to the current position
	int len = strlen(text);
//  int len = atoi(str_cloude.length()) - 3;
